...
wandb.init(project='writing_task_class_ai_coh', entity='knebhi')
...
def prepare_data(csv_path="data/coh_data.csv"):
...
def detailed_evaluation(trainer, tokenized_valid):
    ...
    pd.DataFrame(results_tasks).to_csv("result_eval_data_roberta_large_writing_task_coh.csv", index=False)
    pd.DataFrame(results_levels).to_csv("result_eval_data_roberta_large_coh_by_level.csv", index=False)
...
def main():
    ...
    output_dir = "model_saved/roberta-large-ft-coh-writing-task-augmented"
    ...
    upload_model_to_s3("model_saved/roberta-large-ft-coh-writing-task-augmented", bucket_name, "writing_task_models/coherence/")
