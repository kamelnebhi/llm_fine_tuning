...
MAX_LENGTH_ACC     = 512   # accuracy: text_a + correction
MAX_LENGTH_COH_RNG = 256   # coherence/range: text_a seul
...
class MultiTaskCrossEncoderRoberta(nn.Module):
    ...
    def _encode_and_pool(self, input_ids, attention_mask):
        """Shared: encoder → CLS + attn pool → trunk."""
    ...
    def forward(
        self,
        input_ids_acc=None,
        attention_mask_acc=None,
        input_ids_coh_rng=None,
        attention_mask_coh_rng=None,
        labels=None,
        sample_weight=None,
        **kwargs,
    ):
        # ── Passe 1 : accuracy (text_a + correction) ─────────────
        ...
        # ── Passe 2 : coherence + range (text_a seul) ────────────
...
class MultiTaskOrdinalTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # inputs contient maintenant les 4 tenseurs :
        # input_ids_acc, attention_mask_acc, input_ids_coh_rng, attention_mask_coh_rng
        ...
        loss_a = self._ordinal_loss(
            logits[:, :NUM_LABELS], labels[:, 0], sw_a, sigma=self.sigma[0],
        )
        loss_c = self._ordinal_loss(
            logits[:, NUM_LABELS:2*NUM_LABELS], labels[:, 1], sw_c, sigma=self.sigma[1],
        )
        loss_r = self._ordinal_loss(
            logits[:, 2*NUM_LABELS:], labels[:, 2], sw_r, sigma=self.sigma[2],
        )
...
# TOKENIZATION
# ============================================================
def tokenize_ds(ds: Dataset, tokenizer, max_length_acc: int, max_length_coh_rng: int) -> Dataset:
    def _tokenize(examples):
        # Passe 1 : accuracy (text_a + text_b)
        enc_acc = tokenizer(
            examples["text_a"],
            examples["text_b"],
            truncation=True,
        ...
        return {
            "input_ids_acc":            enc_acc["input_ids"],
            "attention_mask_acc":       enc_acc["attention_mask"],
            "input_ids_coh_rng":        enc_coh_rng["input_ids"],
            "attention_mask_coh_rng":   enc_coh_rng["attention_mask"],
        }
...
def main():
    ...
    val_ds, test_ds, eval_orig_indices, _ = build_fixed_eval_sets(
    ...
    print(f"\nTokenizing eval sets (acc={MAX_LENGTH_ACC}, coh_rng={MAX_LENGTH_COH_RNG}) …")
    tokenized_test  = tokenize_ds(test_ds,  tokenizer, MAX_LENGTH_ACC, MAX_LENGTH_COH_RNG)
    tokenized_valid = tokenize_ds(val_ds,   tokenizer, MAX_LENGTH_ACC, MAX_LENGTH_COH_RNG)

    experiments = [{"name": "multitask_cross_encoder_v4"}]
    ...
    for exp in experiments:
        ...
        train_ds = prepare_train_set(
            CSV_PATH, eval_orig_indices,
            agreement_threshold=60,
            experiment_dir=f"data_v2/{exp_name}",
            threshold_accuracy=60, threshold_coherence=60, threshold_range=60,
        )

        tokenized_train = tokenize_ds(train_ds, tokenizer, MAX_LENGTH_ACC, MAX_LENGTH_COH_RNG)
