...
class MultiTaskCrossEncoderRoberta(nn.Module):
    """
    Multitask cross-encoder avec :
    - Shared trunk  : CLS + mean pooling → [batch, 768]
    - Task-specific projections (spécialisation par tâche)
    - Cross-task residuals accuracy → coherence / range
    - log_sigma appris : uncertainty weighting (Kendall et al., 2018)
    """
    def __init__(self, model_name: str, num_labels: int, dropout: float = 0.1):
        ...
        proj_dim = 256
        self.proj_accuracy  = self._make_proj(768, proj_dim, dropout)
        self.proj_coherence = self._make_proj(768, proj_dim, dropout)
        self.proj_range     = self._make_proj(768, proj_dim, dropout)

        # ── Classification heads ───────────────────────────────────────
        self.head_accuracy  = nn.Linear(proj_dim, num_labels)
        self.head_coherence = nn.Linear(proj_dim, num_labels)
        self.head_range     = nn.Linear(proj_dim, num_labels)

        # ── Cross-task residual (corrélation acc→coh et acc→rng) ───────
        self.cross_acc_to_coh = nn.Linear(proj_dim, proj_dim, bias=False)
        self.cross_acc_to_rng = nn.Linear(proj_dim, proj_dim, bias=False)
        nn.init.eye_(self.cross_acc_to_coh.weight)   # init identité
        nn.init.eye_(self.cross_acc_to_rng.weight)

        # ── Learnable log-sigma (uncertainty weighting) ────────────────
        # log_sigma[0]=accuracy  [1]=coherence  [2]=range
        # Appris pendant le training → device géré automatiquement
    ...
    def forward(
        ...
        **kwargs,
        ...
        mean_repr     = (encoder_out.last_hidden_state * mask_expanded).sum(1) \
                        / mask_expanded.sum(1).clamp(min=1e-9)

        combined = cls_repr + mean_repr   # [batch, 1024]

        # Shared trunk
        shared = self.shared_trunk(combined)   # [batch, 768]

        # Task-specific projections
        feat_acc = self.proj_accuracy(shared)    # [batch, 256]
        feat_coh = self.proj_coherence(shared)   # [batch, 256]
        feat_rng = self.proj_range(shared)       # [batch, 256]

        # Cross-task residual (α=0.1 pour ne pas dominer la proj spécifique)
        feat_coh = feat_coh + 0.1 * self.cross_acc_to_coh(feat_acc)
        feat_rng = feat_rng + 0.1 * self.cross_acc_to_rng(feat_acc)
...
# ============================================================
# MULTITASK ORDINAL TRAINER — Uncertainty weighting automatique
# ============================================================
class MultiTaskOrdinalTrainer(Trainer):
    """
    - Ordinal soft-label loss par tâche (sigma fixe = largeur des soft labels)
    - Uncertainty weighting via model.log_sigma (appris en même temps)
      L = sum_i [ exp(-2*log_sigma_i) * L_i + log_sigma_i ]
      → les tâches intrinsèquement incertaines reçoivent automatiquement
        un poids plus faible sans hyper-paramètre manuel
    """
    def __init__(self, *args, sigma=None, **kwargs):
        super().__init__(*args, **kwargs)
        # sigma fixe : largeur des soft labels par tâche (pas appris)
        if sigma is None:
            sigma = [0.8, 1.5, 1.2]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # model.log_sigma est un nn.Parameter → même device que le modèle
        ...
        # Monitoring des sigmas appris toutes les 100 steps
        if self.state.global_step % 100 == 0 and model.training:
            s = torch.exp(log_sigma).detach().cpu().numpy()
            print(f"\n  [step {self.state.global_step}] task sigmas — "
                  f"acc={s[0]:.3f}  coh={s[1]:.3f}  rng={s[2]:.3f}")
...
def preprocess_df(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df.dropna(subset=['label_accuracy', 'label_coherence', 'label_range',
                      'text_a', 'text_b'], inplace=True)
...
# ============================================================
# FIXED EVAL SETS — filtrés sur les 3 tâches
# ============================================================
def build_fixed_eval_sets(
    ...
    df["agreement_percentage_coherence"] = pd.to_numeric(
        df["agreement_percentage_coherence"], errors='coerce')
    df["agreement_percentage_range"]     = pd.to_numeric(
        df["agreement_percentage_range"],     errors='coerce')

    print("  Computing max_length from accuracy>=60% pool …")
    df_pool      = df[df["agreement_percentage_accuracy"] >= eval_threshold_accuracy]
    ...
    # ── Filtre fiabilité sur les 3 tâches ─────────────────────────────
    # Seuls les samples avec des labels fiables pour TOUTES les tâches
    # entrent dans l'eval → métriques coherence/range exploitables
    df_reliable = df[
...
def prepare_train_set(
    ...
    # Seuils spécifiques par tâche, fallback sur agreement_threshold global
...
# ============================================================
# METRICS — composite QWK + per task
# ============================================================
def compute_metrics(eval_pred):
    ...
    # Composite metric — moyenne des QWK des 3 tâches
    # Utilisé pour sauvegarder le meilleur checkpoint global
...
def train_model(tokenized_train, tokenized_test, num_labels, output_dir, tokenizer):
    ...
    training_args = TrainingArguments(
        ...
        metric_for_best_model="composite_qwk",   # ← moyenne QWK des 3 tâches
    ...
    trainer = MultiTaskOrdinalTrainer(
        ...
        sigma=[0.7, 0.9, 0.85],   # soft-label width fixe par tâche
    )
...
def main():
    ...
    AGREEMENT_THRESHOLD = 60   # fallback global si seuils spécifiques non fournis
