...
class AttentionPooling(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        scores = self.attn(hidden_states).squeeze(-1)  # ← apply Linear first
        scores = scores.masked_fill(attention_mask == 0, -1e4)
        weights = torch.softmax(scores, dim=-1).unsqueeze(-1)
        return (hidden_states * weights).sum(dim=1)

# ============================================================
# TASK ADAPTER (lightweight bottleneck per task)
# ============================================================
class TaskAdapter(nn.Module):
    """Bottleneck adapter for task specialization on shared trunk."""
...
# ============================================================
# MULTITASK CROSS-ENCODER MODEL (v2 — improved)
# ============================================================
class MultiTaskCrossEncoderRoberta(nn.Module):
    """
    Multitask cross-encoder v2 :
    - Concat pooling (CLS + attention pooling) → [batch, 2*hidden]
    - Shared trunk → 768
    - Per-task adapters (lightweight bottleneck)
    - Asymmetric task projections (more capacity for harder tasks)
    - No cross-task residual (removed — was injecting noise)
    - Learnable log_sigma for uncertainty weighting (Kendall et al., 2018)
    """
    def __init__(self, model_name: str, num_labels: int, dropout: float = 0.1):
        ...
        # ── Shared trunk (concat pooling: CLS + attn → 2*hidden) ──────
        ...
        # ── Per-task adapters on shared trunk ──────────────────────────
        ...
        # ── Asymmetric task projections ────────────────────────────────
        # Coherence/range are harder → more capacity
        proj_dim_acc = 256
        proj_dim_coh = 384
        proj_dim_rng = 320
        ...
        # ── Learnable log-sigma (uncertainty weighting) ────────────────
        self.log_sigma = nn.Parameter(torch.zeros(3))

    @staticmethod
    def _make_proj(in_dim: int, out_dim: int, dropout: float):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # ── Concat pooling: CLS + attention pooling ───────────────────
        cls_repr  = encoder_out.last_hidden_state[:, 0, :]
        attn_repr = self.attn_pool(
            encoder_out.last_hidden_state, attention_mask
        )
        combined = torch.cat([cls_repr, attn_repr], dim=-1)  # [B, 2048]

        # ── Shared trunk ──────────────────────────────────────────────
        shared = self.shared_trunk(combined)   # [B, 768]

        # ── Per-task adapter + projection (no cross-task residual) ────
        feat_acc = self.proj_accuracy(self.adapter_accuracy(shared))
        feat_coh = self.proj_coherence(self.adapter_coherence(shared))
        feat_rng = self.proj_range(self.adapter_range(shared))

        logits = torch.cat([
            self.head_accuracy(feat_acc),
            self.head_coherence(feat_coh),
            self.head_range(feat_rng),
        ], dim=-1)   # [B, 18]
...
# ============================================================
# MULTITASK ORDINAL TRAINER — v2 with differential LR
# ============================================================
class MultiTaskOrdinalTrainer(Trainer):
    """
    - Ordinal soft-label loss per task (fixed sigma = label smoothing width)
    - Uncertainty weighting via model.log_sigma (learned jointly)
    - Differential learning rate: encoder (slow) vs heads (fast)
    """
    ...
    def create_optimizer(self):
        ...
        print(f"\n  [Optimizer] encoder LR={self.encoder_lr}, "
              f"head LR={self.head_lr}")
        print(f"  [Optimizer] encoder params={len(encoder_params)}, "
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        loss_a = self._ordinal_loss(
            logits[:, :NUM_LABELS],
            labels[:, 0], sw_a, sigma=self.sigma[0],
        )
        loss_c = self._ordinal_loss(
            logits[:, NUM_LABELS:2*NUM_LABELS],
            labels[:, 1], sw_c, sigma=self.sigma[1],
        )
        loss_r = self._ordinal_loss(
            logits[:, 2*NUM_LABELS:],
        ...
        if self.state.global_step % 100 == 0 and model.training:
            ...
            print(
                ...
                f"task sigmas — acc={s[0]:.3f}  coh={s[1]:.3f}  rng={s[2]:.3f}"
                f"  |  effective weights — acc={w[0]:.3f}  coh={w[1]:.3f}  rng={w[2]:.3f}"
...
def preprocess_df(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    keep = [
        "orig_idx", "text_a", "text_b", "task_id", "ef_level",
    ...
    df.dropna(
        subset=['label_accuracy', 'label_coherence', 'label_range',
                'text_a', 'text_b'],
        inplace=True,
    )
...
def build_fixed_eval_sets(
    ...
    eval_threshold_coherence: int = 50,
    eval_threshold_range:     int = 50,
):
    ...
    print("  Computing max_length from accuracy>=60% pool …")
    df_pool      = df[
    ...
    # ── Reliability filter on all 3 tasks ─────────────────────────────
    df_reliable = df[
        (df["agreement_percentage_accuracy"]  >= eval_threshold_accuracy)  &
    ...
    print(f"\n  After reliability filter "
          f"(acc>={eval_threshold_accuracy}%, "
          f"coh>={eval_threshold_coherence}%, "
          f"rng>={eval_threshold_range}%): "
          f"{len(df_reliable)} samples")

    df_100 = df_reliable[
        df_reliable["agreement_percentage_accuracy"] == 100
    ].copy()
    df_60_80 = df_reliable[
        (df_reliable["agreement_percentage_accuracy"] >= eval_threshold_accuracy)
        & (df_reliable["agreement_percentage_accuracy"] <= 80)
    ].copy()
        ...
        return (
            df_in.groupby('label_accuracy', group_keys=False)
                 .apply(lambda x: x.sample(
                     max(1, int(len(x) * ratio)), random_state=seed
                 ))
        )
    ...
    df_eval["is_100_agreement"] = df_eval["orig_idx"].isin(
        final_100["orig_idx"]
    )
...
def prepare_train_set(
    ...
    def _compute_task_weight(df, agreement_col, label_col):
        agr = (
            pd.to_numeric(df[agreement_col], errors='coerce').fillna(0.5)
            / 100.0
        )
        ...
        total              = len(df)
        cw_map = {
            cls: total / (n_classes_present * cnt)
    ...
    thr_acc = (threshold_accuracy  if threshold_accuracy  is not None
               else agreement_threshold)
    thr_coh = (threshold_coherence if threshold_coherence is not None
               else agreement_threshold)
    thr_rng = (threshold_range     if threshold_range     is not None
               else agreement_threshold)
    ...
    df_train = df_thresh[
        ~df_thresh['orig_idx'].isin(eval_orig_indices)
    ].copy()
    ...
    # ── Independent sample weights per task ────────────────────────────
    ...
    print(
        f"\n[Train | accuracy>={thr_acc}% | coherence>={thr_coh}% "
        f"| range>={thr_rng}%]  n_samples={len(df_train)}"
    )
    for task in TASK_NAMES:
        lbl = f"label_{task}"
        print(f"\nLabel {task} distribution:")
        print(df_train[lbl].value_counts().sort_index())
...
# ============================================================
# METRICS — harmonic mean composite QWK
# ============================================================
def compute_metrics(eval_pred):
    ...
    for i, task in enumerate(TASK_NAMES):
        ...
        try:
            if len(set(task_labels)) > 1 and len(set(task_preds)) > 1:
                ...
                prs, _     = pearsonr(task_labels, task_preds)
                adj_acc    = (
                    np.abs(task_preds.astype(int) - task_labels.astype(int)) <= 1
                ).mean()

                metrics[f"{task}_accuracy"]    = acc
                metrics[f"{task}_cohen_kappa"] = qwk
                metrics[f"{task}_pearson"]     = prs
                metrics[f"{task}_adj_acc"]     = adj_acc
                qwk_scores.append(qwk)
            else:
                metrics[f"{task}_accuracy"]    = float('nan')
                metrics[f"{task}_cohen_kappa"] = float('nan')
                metrics[f"{task}_pearson"]     = float('nan')
                metrics[f"{task}_adj_acc"]     = float('nan')
        except Exception as e:
            print(f"[!] Metrics error for {task}: {e}")

    # ── Harmonic mean of QWK — penalizes weak tasks ───────────────────
    if qwk_scores and all(q > 0 for q in qwk_scores):
        ...
        metrics["composite_qwk"] = float(
            np.mean(qwk_scores) if qwk_scores else float('nan')
        )
...
def safe_metrics(ref_labels, predicted_labels):
    ...
    try:
        result["ck"] = round(
            cohen_kappa_score(
                predicted_labels, ref_labels, weights="quadratic"
            ), 4
        )
    except Exception as e:
    ...
    try:
        result["accuracy"] = round(
            accuracy_score(ref_labels, predicted_labels), 4
        )
        p, r, f, _ = precision_recall_fscore_support(
            ref_labels, predicted_labels,
...
def train_model(
    ...
    trainer = MultiTaskOrdinalTrainer(
        ...
        sigma=[0.7, 0.9, 0.85],     # tighter soft labels for coh/rng
        encoder_lr=1e-5,             # slow encoder
        head_lr=3e-4,                # fast heads + adapters + projections
    )
...
def detailed_evaluation(
    ...
    for i, task in enumerate(TASK_NAMES):
        ...
        qwk        = cohen_kappa_score(
            task_refs, task_predicted, weights="quadratic"
        )
        lwk        = cohen_kappa_score(
            task_refs, task_predicted, weights="linear"
        )
        pearson, _ = pearsonr(task_refs, task_predicted)
        adj_acc    = (
            np.abs(task_predicted.astype(int) - task_refs.astype(int)) <= 1
        ).mean()
        mae = np.abs(
            task_predicted.astype(int) - task_refs.astype(int)
        ).mean()
        ...
        cm_norm_df.to_csv(
            f"{out_dir}/confusion_matrix_{task}_normalized.csv"
        )
    ...
    for idx in range(len(tokenized_valid)):
        ...
        for i, task in enumerate(TASK_NAMES):
            ...
            probs = torch.softmax(
                torch.tensor(task_logits), dim=-1
            ).numpy()
...
def main():
    ...
    for exp in experiments:
        ...
        trainer, eval_results = train_model(
            tokenized_train, tokenized_test, NUM_LABELS,
