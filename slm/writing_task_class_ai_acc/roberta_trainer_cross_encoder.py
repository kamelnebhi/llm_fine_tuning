...
# ============================================================
# CROSS-ENCODER MODEL  (single RoBERTa-large encoder)
# ============================================================
class CrossEncoderRoberta(nn.Module):
    """
    Single-encoder cross-encoder model.
    Input : [CLS] text_a [SEP] text_b [EOS]
    text_a = level + prompt + student response
    text_b = expert correction
    Head   : CLS → Dropout → 1024 → 512 → num_labels
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Single forward pass — both texts see each other via attention
        cls = self.encoder(
            input_ids=input_ids,
...
# ============================================================
# DATA COLLATOR  — single tower fields + optional sample_weight
# ============================================================
...
class OrdinalTrainer(Trainer):
    ...
    def _load_best_model(self):
        """
        Override needed for custom nn.Module — HuggingFace Trainer
        cannot auto-reload a non-PreTrainedModel checkpoint.
        """
        best_ckpt = self.state.best_model_checkpoint
        if best_ckpt is None:
            print("[!] No best checkpoint found — keeping current model weights.")
            return
...
# ============================================================
# SHARED TEXT PREPROCESSING  — produces text_a / text_b
# ============================================================
def preprocess_df(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # text_a — context + student response
    df['text_a'] = (
...
# ============================================================
# TOKENIZATION  — single cross-encoder input
# ============================================================
def tokenize_ds(ds: Dataset, tokenizer, max_length: int) -> Dataset:
    """
    text_a and text_b are passed as a sentence pair.
    RoBERTa encodes them as:
        [CLS] text_a [SEP] text_b [EOS]
    Both texts attend to each other across the full sequence.
    Truncation strategy : 'only_first' preserves the correction (text_b)
    entirely and truncates text_a (level + prompt + response) if needed.
    """
    def _tokenize(examples):
        enc = tokenizer(
            examples["text_a"],
            examples["text_b"],
            truncation=True,   # protect correction (text_b)
            padding="max_length",
...
def build_fixed_eval_sets(csv_path: str, tokenizer, force_rebuild: bool = False):
    ...
    # ── max_length: p95 of combined pair lengths ───────────────────────────
    print("  Computing max_length from agreement>=60% pool …")
    df_pool = df[df["agreement_percentage"] >= 60]

    # Tokenize as pairs to get realistic combined lengths
...
def main():
    ...
    experiments = [
        {"agreement_threshold": AGREEMENT_THRESHOLD,
         "name": f"cross_encoder_gte{AGREEMENT_THRESHOLD}"},
    ]
    ...
    for exp in experiments:
        ...
        exp_name     = exp["name"]
        output_dir   = f"model_saved/roberta-large-cross-encoder-{exp_name}"
